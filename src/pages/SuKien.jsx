...
const SuKien = () => {
  ...
  return (
    <div className="max-w-9xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 flex-grow">
          {eventsData.slice(currentIndex, currentIndex + eventsPerPage).map((event) => (
            <div key={event.id} className="bg-white shadow-md rounded-lg overflow-hidden flex m-3 h-80">
              <img src={event.image} alt={event.title} className="w-60 h-78 object-cover" />
      ...
      <div className="flex items-center justify-between mb-4">
        <input type="text" placeholder="Nhập từ khóa tìm kiếm.." className="border w-100 h-10" />
        <ul>
          {
            categories.map((category) => (
              <li key={category.id} className="inline-block mr-4">
                <button className="bg-blue-300 text-white rounded px-4 py-2 hover:bg-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">
                  {category.name}
                </button>
              </li>
            ))
